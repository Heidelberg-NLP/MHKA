...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifiers = nn.Linear(config.dim, 1)#
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None, labels=None):
        ...
        pooled_close = np.zeros((pooled_output.size(0), 2, pooled_output.size(2)), dtype=np.int32) 
        pooled_close = torch.tensor(pooled_close, dtype=torch.float).to(device) 
        pooled_close[: ,0, :] = pooled_output[:, 2, :]
        pooled_close[: ,1, :] = pooled_output[:, 3, :]
        pooled_close = pooled_close.view(-1, pooled_output.size(2))
        #m = nn.Softmax(dim=1)
        m = nn.LogSoftmax()
        logits_sim = m(self.classifiers(pooled_close).view(-1,2))
        #logits = m(self.classifier(pooled_orig).view(-1,2))
        #print('logits', logits)
        #print('logits_close', logits_sim)
        #weight_A = torch.tensor([1.]).requires_grad_()
        #weight_B = torch.tensor([1.]).requires_grad_()
        #w_a = torch.Tensor.exp(weight_A).to(device)
        #w_b = torch.Tensor.exp(self.weight_B).to(device)
        #print(w_a)
        #o = self.classifier(pooled_orig) + self.classifier(pooled_close)
        #o = torch.torch.sigmoid(o)
        logits = self.classifier(pooled_orig) #+ self.classifier(pooled_close)
        #print(logits)
        logits = m(logits.view(-1,2))
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss_fct = nn.KLDivLoss().cuda() # use Kullback-Leibler divergence loss
                print(self.W)
                #print(labels)
                #print(logits.view(-1, 2).size(), labels.size())
                labels = convert_labels_to_one_hot(labels, 2)
                print(labels)
                loss = loss_fct(logits.cuda(), labels.cuda())
                loss_close = loss_fct(logits_sim.cuda(), labels.cuda())
                loss = loss + self.W*loss_close
            outputs = (loss,) + outputs

        return outputs, False, False  # (loss), logits, (hidden_states), (attentions)
